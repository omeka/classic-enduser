...
The [PDF Text plugin](https://omeka.org/classic/plugins/PdfText/){target=_blank} enables searching on PDF files uploaded to an Omeka Classic item by extracting their text layer and saving them to the file records. PDF Text ignores images and layout in the original file, leaving only searchable text.
...
![Configure window with a blue arrow pointing to the configure checkbox](../doc_files/plugin_images/Pdftxtconfig.png)
