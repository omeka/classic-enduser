...
When you install the plugin, you will be automatically directed to the plugin configuration page. You can access this screen again at any time by going to the "Plugins" tab at the top of the admin screen, scrolling down to the OAI-PMH Repository plugin in the list, and clicking the blue "Configure" button.

![Configuration options as described below, with no boxes checked.](../doc_files/plugin_images/oaipmhrepo.png)

At the top of the configuration page, the plugin displays the address for your OAH-PMH repository. 

**Repository name**: A text field where you can enter a name for this OAI-PMH repository. This value is sent as part of the response to an Identify request, and it is how the repository will be identified by harvesters. Defaults to the name of the Omeka installation, but you may wish to enter the name of your institution and/or department.

**Namespace identifier**: A text field to set the namespace identifier; this must be the domain name of your Omeka site. If your site lives in a subdirectory (`yourdomain.com/omeka`), it is the main domain (`yourdomain.com`).  
...
**Expose empty collections**: A checkbox to set whether the plugin should expose empty (public) collections. If enabled, all public collections are included in the ListSets output. If disabled, only collections that actually contain at least one (public) item will be included in the ListSets output. 
...
By editing the plugin's files directly, you can configure some more options about how the repository responds to harvesters. 

Since the default values are recommended for most users, these values must be edited by hand, in the `config.ini` file in the plugin's directory, accessed using an FTP or SSH client.

![Configuration options as described below, with no boxes checked.](../doc_files/plugin_images/oaipmhconfig.png)
...
**List expiration time**: The amount of time, in minutes, a resumptionToken is valid for. The specification suggests a number in the tens of minutes. This is the length of time a harvester has to request the next part of an incomplete request. Default is "10" (minutes).

### Extensions

The plugin provides a filter that other plugins can use to add new metadata formats or replace the existing ones with new implementations. As of plugin version 2.1, it's no longer necessary to add or change files within the plugin itself to change the available formats.
