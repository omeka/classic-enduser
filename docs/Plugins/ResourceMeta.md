...
The [Resource Meta plugin](https://omeka.org/classic/plugins/ResourceMeta/){target=_blank} will output resource metadata (elements of items, item sets, and media) as [`<meta>` elements in the HTML](https://www.w3schools.com/tags/tag_meta.asp){target=_blank} of public site pages. 

For example, a metadata property, such as `dcterms:creator`, can be set to output as a meta tag, such as `<meta name="dcterms.creator" content="Shakespeare, William">` inside the `<head>` tag of the public page. This improves indexing and discoverability of the materials on your site. 
...
## Map metadata to meta tags

Select "Resource Meta" from the Plugins list in the left-hand menu of the administrative dashboard. All of the installation's element sets are listed on the page, including Dublin Core and item-type metadata. 
...
If you wish your Omeka items to be [indexed and imported by Zotero](https://zotero-manual.github.io/adding-items/#generic-translators){target=_blank}, you should use Highwire Press, Dublin Core, and PRISM. 
