...
The [Omeka API Import plugin](https://omeka.org/classic/plugins/OmekaApiImport/){target=_blank} allows users to import item data and users from an existing Omeka Classic installation (the "origin") via the API into their own Omeka Classic installation (the "target"). 
