...
With the [Zotero Import plugin](https://omeka.org/classic/plugins/ZoteroImport/){target=_blank} you can move your Zotero research into Omeka Classic, where you can exhibit your Zotero library. Sync your library to the Zotero server, tell the Zotero Import plugin which library you want to import, and it will pull in items, notes, files, and web snapshots. This plugin imports Zotero libraries and collections. 

To minimize data loss that often occurs when migrating between systems, the plugin adds a new element set to your Omeka installation which contains elements identical to the fields used by Zotero.

In addition to mapping data to the Zotero element set, the plugin maps data to the native Dublin Core element set to maximize interoperability. The Zotero element set is large, but the plugin provides a helper function to build strings from the elements.
...
Once you've [installed](../Admin/Adding_and_Managing_Plugins.md) the plugin, go to the "Zotero Import" link in the left-hand sidebar of the admin interface and fill out the form (`/admin/zotero-import`).
...
1. Depending on the size of the library, the import process may take some time to complete. Because of this we suggest that you log background processes:
    ...
    - These steps are not required but will be helpful if something goes wrong halfway through a long import. The import process is only as stable as the Zotero API. If you encounter errors, delete the import and try again.
