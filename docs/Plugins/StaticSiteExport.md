...
After installing and configuring this plugin, click on "Static Site Export" in the left-hand navigation, and click the "Export a site" button.
...
- declaring whether to "Include private resources": If this is left unchecked, your site will include only resources that are available to logged-out site viewers. If checked, resources only visible to logged-in users will also be made available in the export (as though they were public). 
...
![The plugin Export page showing three options, as described above, and an "Export" button.](../doc_files/plugin_images/StaticSiteExport_export.png)

After configuring the export, click the "Begin export" button. The new export will be the first in the table on the base plugin page. The export is finished once the status is marked as "completed".
...
An export may take a long time, depending on the size of the site. The export will contain all file derivatives (including original uploads) for all items. 
...
The export will be a ZIP file that appears in the folder you set in the plugin configuration. You can navigate to and download your export ZIP using your browser or with the command line. 
...
You can delete exports from the table with the "Delete" link in each row. This will delete the export file as well as the database record. 
