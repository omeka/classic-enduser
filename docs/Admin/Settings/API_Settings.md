...
Versions 2.1 and above include an [application programming interface](http://en.wikipedia.org/wiki/Application_programming_interface){target=_blank} (API) that allows site administrators to make their item, collection, tag, and exhibit data visible and usable by outside applications. 

Some online collections making their data available through APIs include the [Digital Public Library of America](http://dp.la/info/developers/codex/){target=_blank} and the [Brooklyn Museum](http://www.brooklynmuseum.org/opencollection/api/){target=_blank}. 

APIs make your collections more accessible, and also contribute to long-term sustainability by making it easier for your Omeka.net data to communicate and be shared with other systems via web-based protocols.
